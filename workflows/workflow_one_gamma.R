##Function that performs the entire workflow for the one step calibration process with the gamma distribution
...
workflow_one_gamma <- function(ensemble_name,
                              control_name,
                              obs_name,
                              power_name,
                              file_path,
  ...
  source("~/Work/Gits/ensemble_post_processing/calibration/emos_power_ensembles_gamma.R")
  
  emos_parameters <- emos_power_ensembles(power_ensembles = raw_ens_prediction, 
                                          horizons = horizons,
                                          daysToTrain = emosDaysToTrain)
  
  distribution_parameters <- emos_power_distribution(emos_params = emos_parameters, 
  ...
  cal_ens_prediction <- emos_power_forecast(emos_params = emos_parameters, 
                                            power_ensembles = raw_ens_prediction, 
                                            horizons = horizons,
                                            daysToTrain = emosDaysToTrain)
